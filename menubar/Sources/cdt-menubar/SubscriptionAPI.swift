...
// (e.g. `"seven_day_sonnet":{"utilization":0.0,"resets_at":null}` — a JSON null for resets_at). The
// previous model made these fields non-optional, so ONE null aborted the entire decode with
// "The data couldn't be read because it is missing." — discarding the perfectly-valid session/weekly
// numbers and leaving the menu bar stuck on "unavailable" forever. Every field is now tolerant: a
// missing/null/wrong-typed field becomes nil and can never break the read of the fields that ARE valid.
private struct OAuthUsageResponse: Decodable {
    struct Period: Decodable {
        ...
        // Fully fault-tolerant: each field decodes independently with `try?`, so a null or unexpectedly
        // typed value yields nil instead of throwing and taking the whole response down with it.
    ...
    // Each period decodes independently — a malformed period can't abort sibling periods.
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        ...
        sevenDaySonnet = (try? c.decodeIfPresent(Period.self, forKey: .sevenDaySonnet)) ?? nil
    }
}
...
enum UsageError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .http(401): return "token expired — open Claude Code or re-login to refresh"
...
/// Turns a raw `/api/oauth/usage` body into a `SubscriptionUsage`. PURE (no network) so it is unit-tested
/// against captured real payloads. Throws a clean `UsageError` — never a raw decoder error — and refuses
/// to fabricate a "0%" reading from a body it couldn't actually parse.
func parseUsageResponse(_ data: Data, account: ClaudeAccount) throws -> SubscriptionUsage {
    ...
    return SubscriptionUsage(
        ...
        sonnetPct: r.sevenDaySonnet?.utilization.map { Int($0.rounded()) },
...
/// Fetches the real subscription usage. Throws `UsageError` on missing token / network / non-200 / decode.
...
func fetchSubscriptionUsage() async throws -> SubscriptionUsage {
    ...
    return try parseUsageResponse(data, account: account)
}
